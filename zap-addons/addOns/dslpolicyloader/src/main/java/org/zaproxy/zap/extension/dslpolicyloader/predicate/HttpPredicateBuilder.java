    ...
    public Predicate<HttpMessage> build(
        ...
        return new Predicate<HttpMessage>() {
            ...
            private String getField(HttpMessage msg) {
                String field;
                if (transmissionType_ == TransmissionType.REQUEST
                        && fieldType_ == FieldType.HEADER) {
                    field = msg.getRequestHeader().toString();
                } else if (transmissionType_ == TransmissionType.REQUEST
                        && fieldType_ == FieldType.BODY) {
                    field = msg.getRequestBody().toString();
                } else if (transmissionType_ == TransmissionType.RESPONSE
                        && fieldType_ == FieldType.HEADER) {
